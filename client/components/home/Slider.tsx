...
const HomeHero = () => {
  ...
  return (
    <section className="w-full h-full relative bg-gray-200 dark:bg-gray-800">
      ...
      {/* left */}
      <button
        className="z-40 absolute top-1/2  -translate-y-1/2 md:left-10 left-4"
        onClick={() => handlePrevious()}
      >
        <FaAngleLeft className="w-12 text-white h-12 rounded-full bg-black/20 p-2 z-40 px-2" />
      </button>
      {/* right */}
      <button
        className="z-40 absolute top-1/2 -translate-y-1/2 md:right-10 right-4"
        onClick={() => handleNext()}
      >
        <FaAngleRight className="w-12 text-white h-12 rounded-full bg-black/20 p-2 z-40 px-2" />
      </button>
    </section>
  );
};
