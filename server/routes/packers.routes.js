...
router.post("/packerdata", (req, res) => {
  ...
  try {
    db.query(
      "insert into Packersdata values(?,?,?,?,?)",
...
router.post("/updateprice", (req, res) => {
  ...
  try {
    db.query(
      "update Packersdata set price=? where upId=?",
      [price, id],
      (err, rows) => {
        if (err) console.log(err);
        res.send({
...
router.get("/getbycity/:city", (req, res) => {
  ...
  db.query("select * from Packersdata where city=?", [city], (err, rows) => {
    if (err) {
      res.send({
        msg: "no data",
      });
...
router.get("/getiddata/:id", (req, res) => {
  ...
  db.query("select * from Packersdata where userId=?", [id], (err, rows) => {
    if (err) console.log(err);
    res.send({ data: rows });
  });
});
